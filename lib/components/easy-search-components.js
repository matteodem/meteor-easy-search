(function () {
    ...
    Template.esAutosuggest.created = function () {
        var tplScope = this.data,
            id = generateId(tplScope.index, tplScope.id);
    ...
    Template.esAutosuggest.helpers({
        ...
        'snippets' : function (options) {
            ...
            firstFieldValue = this[index.field[0]];
            ...
            return {
                'pre' : parts.shift(),
                'found' : regex.exec(firstFieldValue).shift(),
                'post' : parts.join('l'),
                'value' : firstFieldValue,
                'options' : options
            };
    ...
    Template.esAutosuggest.events({
        ...
        'click .remove' : function () {
            ...
            SearchVars.set(this.id, { 'autosuggestSelected' :  _.without(
                SearchVars.get(this.id, 'autosuggestSelected'), this.value) 
            });
        }
    });
