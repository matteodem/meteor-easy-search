EasySearch.getComponentInstance = (function () {
  ...
  function esInputSearch (conf) {
    var id = conf.easySearchID,
      indexConf = EasySearch.getIndex(conf.easySearchIndex),
    ...
    if (searchValue.length < 1) {
      ...
      if (indexConf.reactive) {
        newSubscription = Meteor.subscribe(indexConf.collection._name + '/easySearch', searchValue);
      }
  ...
  function triggerSearch(index, componentId) {
    ...
    if (conf.reactive) {
      var res = conf.collection.find({}, { sort: conf.sort(searchValue) });

      SearchVars.set(id, {
        'searching' : false,
        'total' : res.count(),
        'searchingDone' : true,
        'searchResults' : _.first(res.fetch(), conf.limit),
