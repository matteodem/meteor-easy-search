EasySearch.getComponentInstance = (function () {
  ...
  function easySearch(opts) {
    ...
    if (searchValue.length < 1 && typeof searchValue !== 'object'  && !opts.allDocsOnEmpty) {
  ...
  function triggerSearch(index, componentId) {
    ...
    if ((!searchValue || !searchValue.length) && typeof searchValue !== 'object' && !allDocsOnEmpty) {
      return;
    }

    //for compatible mode with search by object
  ...
  Template.esPagination.helpers({
    ...
    'control' : function () {
      ...
      if(isNaN(currentControl)){
        currentControl = 1;
      }
      //... if too mush sites
      var sitesStarted = 1;
      var sitesStopped = sites;
      if(sites > 10){
        if(currentControl > 4){
          sitesStarted = currentControl - 3;
          controls.push({ text: '1', state: '', action: 1 });
        }else{
          sitesStarted = 1;
        }

        sitesStopped = currentControl + 3;
        if(sitesStopped > sites){
          sitesStopped = sites;
        }
      }

      for (i = sitesStarted; i <= sitesStopped; i += 1) {
        control = { text: i.toString(), state: '', action: i };
      ...
      if(sitesStopped < sites){
        controls.push({ text: sites, state: '', action: sites });
      }

      control = { text: 'Next', state: '', action: EasySearch.PAGINATION_NEXT };
  ...
  return function (conf) {
    return {
      ...
      search : function (value) {
        ...
        // set up default limit
        SearchVars.set(id, { 'defaultLimit' : conf.limit });
        SearchVars.set(id, { allDocsOnEmpty: conf.allDocsOnEmpty });
      ...
      manageAutosuggestValues : function(callback) {
        ...
        check(current, [{ id: String, value: String }]);
