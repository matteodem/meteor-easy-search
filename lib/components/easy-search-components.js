EasySearch.getComponentInstance = (function () {
  ...
  function triggerSearch(index, componentId) {
    ...
    if (conf.reactive && conf.use !== 'minimongo') {
    ...
    if (conf.reactive && conf.use !== 'minimongo') {
      Deps.autorun(function () {
        var searchValue = SearchVars.get(id, 'currentValue'),
          res = conf.collection.esFind({ sort: conf.sort(searchValue) });
  ...
  Template.esInput.created = function () {
    this.autorun(function () {
      ...
      data.index = !_.isArray(data.index) ? [data.index] : data.index;

      _.each(data.index, function (index) {
        var id = generateId(index, data.id);

        // set up default limit
        SearchVars.set(id, { 'defaultLimit' : EasySearch.getIndex(index).limit });
        createSearchDependency(index, data.id);
        Meteor.subscribe(index + '/easySearchCount');
      });
    });
  };
  ...
  Template.esEach.helpers({
    'elasticSearchDoc' : function () {
      ...
      if (conf.reactive && conf.use !== 'minimongo') {
        return conf.collection.esFind({ sort: conf.sort() });
      }
  ...
  Template.ifEsIsSearching.helpers({
    'isSearching' : function () {
      ...
      if (!_.isArray(tplScope.index)) {
        tplScope.index = [tplScope.index];
      }

      return _[combineMethod](_.map(tplScope.index, function (index) {
