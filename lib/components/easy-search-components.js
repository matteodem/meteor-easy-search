EasySearch.getComponentInstance = (function () {
  ...
  return function (conf) {
    return {
      ...
      addToAutosuggestSelected : function(item, removeDuplicates) {
        ...
        // optionally remove items with dupicate ids to prevent unintended behaviors
        if (removeDuplicates) {
          var existing = [];

          current = _.filter(current, function (object) {
            if ( _.contains(existing, object.id) ) {
              return false;
            } else {
