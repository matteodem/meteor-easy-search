EasySearch.getComponentInstance = (function () {
  ...
  function createSearchDependency(index, id) {
  ...
  Template.esInput.created = function () {
    ...
    _.each(inputScope.data.index, function (index) {
      var id = generateId(index, inputScope.data.id);

      createSearchDependency(index, id);
    });
  };
  ...
  return function (conf) {
    return {
      ...
      on : function (key, eventFunction, autorun) {
        var componentScope = this;

        autorun = _.isFunction(autorun) ? autorun : Deps.autorun;
