EasySearch.Components = (function () {
  'use strict';
  ...
  function processSearchResults(id, data, isReactive)
  { 
    // Just store the data
    if (!isReactive) {
      LocalCache.upsert(
        { _id : id },
        { _id : id, r : isReactive, d : data.results }
      );
  ...
  function esInputSearch (conf) {
    ...
    EasySearch.search(index, searchValue, function (err, data) {
      ...
      processSearchResults(id, data, conf.easySearchReactive);
    });
  }
  ...
  function clear(index, componentId) {
    var id = generateId(index, componentId);
    LocalCache.remove({ _id : id });
    SearchVars.set(id, {});
  }
    ...
    'keyup input' : function (e) {
      ...
      if (searchValue.length === 0) {
        _.each(this.index, function (index) {
          clear(generateId(index, eventScope.id));
        });
  ...
  Template.esEach.helpers({
    'elasticSearchDoc' : function () {
      var config,
        ...
        doc = LocalCache.findOne({ '_id' : generateId(this.index, this.id) });
