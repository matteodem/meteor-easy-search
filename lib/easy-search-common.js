      ...
      publishHandle = opts.collection
        ...
        .observe({
          added: function (doc) {
            ...
            if (opts.collection._hasCollectionHelpers) {
              _.each(opts.collection._transform(doc), function (val, key) {
                doc[key] = _.isFunction(val) ? val() : val;
              });
            }
            
            publishScope.added('esSearchResults', doc._id, opts.collection._transform(doc));
          },
