EasySearch = (function () {
  ...
  function setUpPublication(opts) {
    var query, name = opts.collection._name;

    Meteor.publish(name + '/easySearch', function (searchString) {
      var resultArray = [],
        resultSet = Searchers[opts.use].search(name, searchString, indexes[name]);
      ...
      if (resultSet.results.length > 0) {
        ...
        query = opts.collection.find({ _id: { $in: resultArray } });
      } else {
        query = [];
      }

      // todo: also return the count of all results, but not all docs! for esLoadMoreButton
      return query;
    });
  }
