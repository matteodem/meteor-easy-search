var methods = {
  ...
  'search' : function (name, searchString, options, callback) {
    ...
    if (Meteor.isServer) {
      // When running on server, we can improve the performance by computing the total count by using aggregation
      ...
      var aggregates = index.collection.aggregate(pipeline);
